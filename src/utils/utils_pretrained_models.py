...
def set_parameter_requires_grad(model, feature_extracting: bool = True):
    ...
    requires_grad flag. Pretrained models already have trained weights. Their purpose is only to
    extract features. Therefore the gradients are not needed and need to be discarded.
    :param model: Pretrained model with pretrained parameters.
    :param feature_extracting: Purpose of the pretrained model.
    True if only to extract features, false otherwise.
    :return:
    """
...
def get_pretrained_model(
    ...
    :param use_pretrained: When model is loaded, all pretrained parameters will be loaded as well,
    when True.
    :return:
    """
    ...
    if (pretrained_model == PretrainedModels.ResNet) or (
        ...
        raise ValueError(
            'Wrong pretrained model / model name. Check spelling or use predefined enum.')
